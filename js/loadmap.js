mapboxgl.accessToken =
	"pk.eyJ1IjoiZnJhbmtpZTI3ODQiLCJhIjoiY2tnbGdtZzd0MmY2djJ5bDdma3p3eWcxciJ9.nrsHiMn_MItCEKEc9di5cQ";
...
var map = new mapboxgl.Map({
	...
	center: [144.966267, -37.854889], // starting position
...
function closePopup() {
	$(".popup-close").css({
		...
		boxShadow: "0px 0px 0px var(--grey)",
	});
	setTimeout(function () {
...
function positionHighlight(coords) {
	...
	if (window.innerWidth > window.innerHeight || window.innerWidth >= 769) {
		$(".letter-highlight").css({
			transition: "unset",
			top: pixels["y"].toString() + "px",
			left: pixels["x"].toString() + "px",
		});
	} else {
			...
			$(document).ready(function () {
				$(el).hover(
					function (event) {
						...
						$(el).append(
							'<a class="heart-highlight" id="' +
								marker.id +
								'" href="#lightbox" onClick="return loadImages(this)"></a>'
						);
				...
				$(window).on("touchend", function (event) {
					if ($(event.target).closest(el).length) {
						...
						!$(event.target).closest(".divLink-highlight").length &&
						!$(event.target).closest(".marker").length
					) {
