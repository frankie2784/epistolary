...
var borderThickness = Math.min(
	(Math.min(window.innerHeight, window.innerWidth) * 4.0) / 100,
	17
);
var lastSet;
var clicked = false;
...
function loadImages(element) {
	loadJSON(async function (response) {
		...
		if (!isTouchDevice()) {
			imagesHTML = document.getElementsByClassName("lightbox-img");
			imageshidden = document.getElementsByClassName(
				"lightbox-img-hidden"
			);
		} else {
		...
		await transition(currentImage, 0.01);
...
function transition(nextImage, speed) {
	return new Promise((resolve) => {
		...
		function changeOpacity() {
			for (let i = 0; i < numImages; i++) {
				if (i != nextImage) {
					imagesHTML[i].style.opacity = Math.max(
						parseFloat(imagesHTML[i].style.opacity) - del,
						0
					);
				}
			}
			imagesHTML[nextImage].style.opacity = Math.min(
				parseFloat(imagesHTML[nextImage].style.opacity) + del,
				1
			);
...
async function changeImage(element, speed) {
	if (
		numImages > 1 &&
		parseFloat(imagesHTML[currentImage].style.opacity) == 1
	) {
		let imgLeft,
			imgTop,
			imgWidth,
		...
		if (!isTouchDevice()) {
			...
			$(".lightbox-img#transitioning").css({
				height: imgHeight,
				width: imgWidth,
				backgroundSize: imgWidth + "px " + imgHeight + "px",
			});
		...
		await transition(nextImage, speed);
